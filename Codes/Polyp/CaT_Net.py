...
if __name__ == "__main__":
    ...
    network = CaT_Net_with_Decoder_DeepSup(num_classes=2, cnn_channels=32, swin_trans_channels=24, num_layers=[4, 4, 4, 4, 4])
    pred, res = network(input_features)
    params_count = sum(p.numel() for p in network.parameters() if p.requires_grad)
    print(pred.shape)
    print(params_count)
